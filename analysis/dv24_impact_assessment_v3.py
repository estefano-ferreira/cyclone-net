...
def diagnose_defect_0_trailing_only(df):
    """Diagnose shipped bleed: global shift matches ONLY in trailing rows."""
    df = df.copy()
    df = df.sort_values(["sid", "timestamp"], ignore_index=True)
    ...
    report = {
        ...
        "trailing_row_analysis": {
            ...
            "dv24_trailing_global_shift_matches": 0,  # FIXED: trailing only
        },
    ...
    # Global shift matches: ONLY in trailing rows
    df_global = df.sort_values("timestamp", ignore_index=True).copy()
    df_global["dv24_global"] = df_global["wind_kt"].shift(-4) - df_global["wind_kt"]
    ...
    for sid in df["sid"].unique():
        ...
        if len(trailing_indices) > 0:
            trailing = sid_df.iloc[trailing_indices]
            # Map back to global indices to check global shift
            for orig_idx in trailing.index:
                if orig_idx < len(df_global) - 4:
                    if (df.loc[orig_idx, "dv24_shipped"] is not pd.NA and
                        df.loc[orig_idx, "dv24_shipped"] == df_global.loc[orig_idx, "dv24_global"]):
                        report["trailing_row_analysis"]["dv24_trailing_global_shift_matches"] += 1
...
def analyze_irregular_grid(df, splits):
    """Analyze 43 non-6h deltas: SIDs, years, basins."""
    df = df.copy()
    df["timestamp"] = pd.to_datetime(df["timestamp"])
    df = df.sort_values(["sid", "timestamp"], ignore_index=True)
    ...
    report = {
        ...
        "affected_sids_train_val": [],
        "affected_sids_test_count": 0,
    ...
    for sid in df["sid"].unique():
        ...
        if len(sid_df) > 1:
            ...
            if non_6h > 0:
                ...
                # Determine split
                sid_splits = sid_df["sid"].isin(splits[splits["split"].isin(["train", "val"])]["event_id"])
                if sid_splits.any():
                    # This sid has train/val events
                    if not any(s == sid for s in report["affected_sids_train_val"]):
                        report["affected_sids_train_val"].append(sid)
                    ...
                    # Test split only
                    report["affected_sids_test_count"] += 1
...
def main():
    ...
    defect_0_report = diagnose_defect_0_trailing_only(df)
...
def generate_md_report(full_report):
    ...
    md.append(f"- Trailing values matching global shift: {d0['trailing_row_analysis']['dv24_trailing_global_shift_matches']}\n")
    ...
    md.append(f"- Affected SIDs (train+val): {len(ig['affected_sids_train_val'])}\n")
    md.append(f"- Affected SIDs (test only, count): {ig['affected_sids_test_count']}\n")
    if ig['affected_sids_train_val']:
        md.append(f"- SIDs (train+val): {', '.join(ig['affected_sids_train_val'][:10])}")
        if len(ig['affected_sids_train_val']) > 10:
            md.append(f" ... +{len(ig['affected_sids_train_val']) - 10} more\n")
        else:
            md.append("\n")
    md.append(f"- By year: {dict(sorted(ig['deltas_by_years'].items()))}\n")
    md.append(f"- By basin: {ig['deltas_by_basin']}\n\n")
