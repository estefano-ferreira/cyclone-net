...
Hypotheses (channel -> spatial aggregate per time slice):
  ...
  H2 shear:          mean(shear_850_200_mps) level (available 2020-2023 only).
  H3 warm water:     mean(sst_anom_K) level.
  H4 mid humidity:   mean(rh_mid) level (available 2020-2023 only).
...
def main() -> None:
    ...
    audit = {
        ...
        "n_unmatched_onsets": int(n_unmatched),
        ...
        "note": "shear/rh_mid channels exist only for 2020-2023 events (pressure-level coverage)",
    }
