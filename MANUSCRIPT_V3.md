...
Reanalysis-derived tropical-cyclone datasets are an established data-journal genre (Xu et al. 2024, *Earth System Science Data*; Liu et al. 2025, *Scientific Data*), and a reanalysis-derived RI dataset has been released (An & Jeong 2026). This release distributes event cubes (40×40×5×14 per 6-hourly point) with per-event quality-control and supplementary metadata. More critically, it includes frozen, hash-deterministic SID-level splits; tri-state labels with explicit NULL for undefined cases; and complete provenance documentation (per-window SHA-256 manifests, label-correction audit, basin-repair record). These enable storm-level, leakage-safe model development without re-implementing split assignment or storm-level leakage control.
