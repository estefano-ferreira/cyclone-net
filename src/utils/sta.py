...
class SpatialGating(nn.Module):
    """Learn a positive spatial weight map per timestep."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (B, C, T, H, W)
        B, C, T, H, W = x.shape
        x_ = x.permute(0, 2, 1, 3, 4).reshape(B*T, C, H, W)  # (B*T, C, H, W)
        w = self.conv(x_)  # (B*T,1,H,W)
        w = torch.relu(w) + 1e-6
...
class SpatioTemporalAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int = 4, attn_dropout: float = 0.0, proj_dropout: float = 0.0, spatial_gating: bool = True):
        super().__init__()
        self.temporal = TemporalSelfAttention(
            dim, num_heads=num_heads, attn_dropout=attn_dropout, proj_dropout=proj_dropout)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # reshape to sequences: (B*H*W, T, C)
