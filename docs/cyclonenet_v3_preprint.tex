% =====================================================================
% CycloneNet — Preprint (Zenodo record line v3; this compile: v3.0.2)
% Converted from MANUSCRIPT_V3.md (2026-07-16).
% Supersedes the record line v1.0.0 / v1.0.1 / v2.0.0 (corrections: Sec. 9).
% DOI slots resolved to the concept DOI 10.5281/zenodo.18571957
% (conceptdoi read from the Zenodo API for record 21413397, 2026-07-18).
...
two-basin dataset, leakage-safe with respect to storm-identity
non-independence, spanning 1980--2023 (East Pacific and North
...
pressure-level channels contributed no detectable skill (H6: null); under a
global-average-pooling readout and intensity-blind input, the
...
its current form) --- a verdict licensed for that architecture, not for
whether spatial structure carries RI information. This version supersedes
earlier claims in the project's public record line, with full corrections
documented in Section~\ref{sec:correction}.
...
a complete correction record for the project's prior public releases
(Section~\ref{sec:correction}), documenting label revisions (v1$\to$v2),
superseded metrics, and claims refuted or retired. Together, these
...
The dataset is distributed in a single archived record
(10.5281/zenodo.18571957), together with this paper and the repository
code snapshot; the dataset itself is licensed CC~BY~4.0. See
...
MIT. The released data package contains 46 files totaling 6.03~GiB:
\begin{itemize}
...
DATA\_DICTIONARY \S4). The 2020--2023 events carry the same channels from the
original extraction runs --- the initial dataset's 2020--2023 range predates
the backfill --- so the census verifies the union of both extraction passes.
The result
...
plus a logistic-regression linear reference, on the standard information diet
and training protocol (15-epoch budget, identical fold splits, 3 seeds).
Table~\ref{tab:ladder} reports the pooled mean across seeds.
...
\addlinespace
Logistic regression & State + field aggregates (linear reference on the strongest tabular diet) & \textbf{0.203} \\
\bottomrule
\end{tabular}
\end{table}
...
precursor analysis (hypotheses H4/H5 of the registry,
\texttt{docs/hypothesis\_registry.md}; positive verdicts), but their inclusion
in the input set yields no measurable gain when fed to this estimator.
...
\textbf{H9/V2 (architecture justification) was NULL:} on the same underlying
fields --- consumed by the CNN as full spatial grids and by the tabular model
as 44 aggregated scalars (mean/std/min/max of the 11 cube channels) --- the
CNN and GBM$_F$ scored 0.171 and 0.170 respectively, yielding
...
independent of the spatial attribution claim (which hypothesis H1 --- the
pre-registered spatial-localization test, ERRATA item~4 --- had refuted). However,
...
This paper supersedes the project's prior public releases: v1.0.0
...
\textbf{ROC-AUC 0.97 and Recall 0.92}, \textbf{``sub-pixel spatial
accuracy,''} \textbf{``$\sim$26~km mean spatial error,''} and \textbf{``18
hurricanes (1989--2024).''} The records themselves (10.5281/zenodo.18571958,
10.5281/zenodo.18577056) contain the source of these numbers: a
\texttt{validation\_report.txt} and a per-event
\texttt{cyclonenet\_scientific.csv}. Recomputation from the released CSV
(verified 2026-07-17) reproduces every figure in the report exactly ---
ROC-AUC 0.9736, recall 0.9231, precision 0.6857, F1 0.7869, accuracy 0.7759,
Brier 0.1169, mean/median tracking error 25.79/27.80~km --- and identifies
the benchmark's composition: 58 track points from 19 named Atlantic
hurricanes spanning 1989--2024, 18 of which exhibit rapid intensification;
26 RI-positive points against 32 negatives (44.8\% prevalence).

The numbers are withdrawn for scope, not provenance. They were measured on a
58-event benchmark of selected named storms at 44.8\% RI prevalence, with no
confidence intervals and no train/test separation declared in the record, and
were presented as a general characterization of the model. They are not
comparable to any evaluation on the full archive (16{,}780 events, 4.8\%
prevalence) and should not be cited as measures of model skill. ``Target
Lock'' (as branding) was removed at the v2 rewrite; it survives only as
technical descriptor of \texttt{pred\_lat}/\texttt{pred\_lon} in validation
documentation.

The evaluation chain that replaces these figures is documented below (the
v2.0.0 claims and ERRATA item 3).
...
A single archived record --- containing this paper, the dataset package, and
the repository code snapshot --- accompanies this release under the concept
DOI 10.5281/zenodo.18571957 (see Data and code availability).
...
\texttt{LICENSE} file). This paper is accompanied by a single archived record
with a persistent DOI (10.5281/zenodo.18571957, resolving to the latest
version) containing: (1) this paper; (2) the dataset package --- the cubes, sidecars,
splits, labels, and metadata, distributed under CC~BY~4.0 with mandatory
third-party attributions in \texttt{NOTICE}; and (3) the repository snapshot
at tag \texttt{v3.0.2} with the complete configuration and pipeline code
