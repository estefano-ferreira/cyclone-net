...
\date{2026-07-16 \\[0.3em]
\small Preprint --- Zenodo record 10.5281/zenodo.18751255, version 3.0.0}
...
rather than about the practices. The sub-field's evaluation norm is
...
Each component has precedent; the assembled artifact is what is offered. Its
...
architecture} (stages 6--8, right, in grey) is shown for historical
...
components and forensic framing are established, not new. The contribution is
engineering and reproducibility (an auditable, configuration-driven, tested
pipeline), not scientific discovery or architectural novelty. The title term
...
\subsection{Withdrawn claims and retired components}

\begin{itemize}
  \item \textbf{ROC-AUC 0.97, 0.92 recall, ``sub-pixel spatial accuracy,''
  ``$\sim$26~km mean spatial error,'' and ``18 hurricanes (1989--2024)''}: all
  withdrawn. Do not cite.
  \item \textbf{FuelMap localization as a validated result}: withdrawn;
  refuted, not pending.
  \item \textbf{``Atmospheric Singularity Mapping'' and ``Target Lock''} (as
  framing): withdrawn.
  \item \textbf{The CNN architecture}: retired by pre-registered verdict
  (Section~\ref{sec:campaign}, H9/V2).
  \item \textbf{The ``physics-guided'' label}: retired from the project's
  citable identity.
\end{itemize}
...
\slot{author block --- as in Zenodo v3 .tex}

\slot{coauthor slot --- pending contact}
...
% =====================================================================
% REFERENCES — CycloneNet v3 preprint
%
% Estilo: autor-ano com recuo pendente (hanging indent), que casa com as
% citações em texto corrido já usadas no manuscrito ("(Kaplan & DeMaria
% 2003)"). Não usa BibTeX — é autocontido e compila direto no Overleaf.
%
% ESCOPO: só entram obras CITADAS no texto. O literature_review lista mais
% duas revisões (Chen et al. 2020; Wang et al. 2022) que o manuscrito não
% cita — ficam fora de propósito: lista de referências não é bibliografia.
%
% PROCEDÊNCIA: todas verificadas contra a página do editor em
% docs/literature_review.md. Hersbach e Knapp são FONTES DE DADOS (cadeia de
% proveniência própria no NOTICE), não literatura — entram aqui porque o §4
% as cita nominalmente. Gardoll & Boucher, Donno et al. e o DOI do Saeb
% foram verificados nas páginas primárias em 2026-07-16.
% =====================================================================
...
on single levels from 1940 to present. Copernicus Climate Change Service (C3S)
Climate Data Store (CDS).
\href{https://doi.org/10.24381/cds.adbb2d47}{doi:10.24381/cds.adbb2d47}.
\emph{[data source]}

\item Hersbach, H., Bell, B., Berrisford, P., et al. (2018). ERA5 hourly data
on pressure levels from 1940 to present. Copernicus Climate Change Service
