...
UPDATED 2026-07-16 (late): v3 is a full PREPRINT (`MANUSCRIPT_V3.md`), not
a standalone supersession note — the note's content became Section 9 of
the paper. Slots marked ⟦…⟧ wait for the dataset DOI.
...
>
> This version (3.0.0) is a full preprint that SUPERSEDES the entire
...
> reproducible ones (ROC-AUC 0.796 [0.753–0.837], PR-AUC 0.251
> [0.179–0.331] on the frozen 1980–2023 test split); a pre-registered
> campaign (2026-07-16) found no detectable contribution from added
> pressure-level channels (H6 null) and retired the CNN architecture
> (H9: a gradient-boosted tabular baseline beats it, Δ₁ = −0.078
> [−0.116, −0.042]; at a fixed information diet the architecture adds
> nothing, Δ₂ = +0.0005 [−0.029, +0.032]). The "physics-guided" label is
> retired. The project designates no reference model; the validated
> contribution is the auditable dataset and pipeline and the documented
> negative results.
>
...
the preprint PDF + repository snapshot:

- **Record license field: CC BY 4.0** — governs the paper and the
...
content is Section 9 of the paper.)
